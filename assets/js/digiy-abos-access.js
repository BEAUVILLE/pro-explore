/* ============================================================
   DIGIYLYFE · EXPLORE · ABOS ACCESS BRIDGE
   À poser dans : assets/js/digiy-abos-access.js

   EXPLORE garde son identité métier.
   Mais l'abonnement actif en base est EXPLORE_BOOST.
   ============================================================ */
...
(function () {
    ...
    return cleanPhone(
      readQuery("phone") ||
        ...
        readStorage([
          ...
          "DIGIY_SESSION_PHONE",
          "DIGIY_EXPLORE_PHONE",
          "DIGIY_EXPLORE_SESSION_PHONE",
          "DIGIY_EXPLORE_LAST_PHONE",
          "DIGIY_EXPLORE_BOOST_PHONE",
          "DIGIY_DRIVER_PHONE",
          "DIGIY_LOC_PHONE",
          "DIGIY_PAY_PHONE"
        ])
  ...
  function guessModule(options) {
    ...
    const module = upperModule(raw);

    if (module === "EXPLORE") return "EXPLORE_BOOST";
    return module;
  }
  ...
  function savePhone(phone, module) {
    ...
    try {
      ...
      localStorage.setItem("DIGIY_EXPLORE_LAST_PHONE", p);
      localStorage.setItem("DIGIY_EXPLORE_BOOST_PHONE", p);
    } catch (_) {}
  }
  ...
  function renderAccessBadge(target, result) {
    ...
    if (result.has_access) {
      el.innerHTML = `
        <strong>✅ Accès EXPLORE actif</strong><br>
        ${result.fiche_title || "EXPLORE BOOST · Standard"}<br>
        <small>Expire : ${result.expires_at || "date suivie par DIGIY"}</small>
      `;
    } else {
      el.innerHTML = `
        <strong>🔒 EXPLORE à vérifier</strong><br>
        <small>Le module reste EXPLORE, mais l'accès se contrôle via EXPLORE_BOOST.</small>
      `;
    }
  }
