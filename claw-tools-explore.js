...
(() => {
  ...
  const FILE_VERSION = "claw-tools-explore-v1";
  ...
  async function loadPlaceBySlug(slug){
    ...
    try{
      ...
      const { data, error } = await sb.rpc("digiy_explore_public_place_by_slug", {
        p_slug: s
      });
  ...
  async function loadOwnerPlaces(phone){
    ...
    try{
      const sb = await createSupabase();
      const { data, error } = await sb.rpc("digiy_explore_list_places_by_phone", {
        p_owner_phone: p
      });
  ...
  async function loadContext(){
    ...
    let place = null;
    if (slug){
      place = await loadPlaceBySlug(slug);
    }

    let ownerPlaces = [];
    if (phone){
      ownerPlaces = await loadOwnerPlaces(phone);
    }
