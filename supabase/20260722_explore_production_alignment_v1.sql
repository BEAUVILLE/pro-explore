...
-- DIGIY EXPLORE — ALIGNEMENT PRODUCTION V1
-- Objectif : un abonné enregistre sa fiche sans SQL, sans connaître les codes internes.
-- Ne modifie ni les PIN, ni digiy_verify_pin, ni la durée des sessions.

create schema if not exists extensions;
create extension if not exists unaccent with schema extensions;

-- Résout toujours un code réellement autorisé par la clé étrangère.
...
begin
  ...
  -- Le code service existe dans la base actuelle et couvre les activités.
  v_sql := format(
    'select c.%1$I::text
       from public.digiy_explore_categories c
  ...
  -- Dernier secours : première catégorie active réellement présente.
  v_sql := format(
    $query$
      select c.%1$I::text
      from public.digiy_explore_categories c
...
-- Un seul pont canonique entre l'ancien schéma et la nouvelle fiche.
...
-- Contrôles sans créer de nouvel abonné.
select
  public.digiy_explore_resolve_category_code('Pêche / sortie', null)
    as code_category_resolu,
  public.digiy_explore_has_access('221771342889')
    as acces_explore;
...
where owner_slug = 'explore-221771342889';
